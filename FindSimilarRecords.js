 ...
 function beforeAll(graphId, graphModel, graphSelection) {
   ...
   .then(function (entities) {
     ...
     entities = entities.filter(function(entry){
        // console.log(entry)
         return entry.type === "SAVED_SEARCH" && entry.label != ".siren" && !entry.label.includes("-virtual");
     })
     console.log(entities)
     ...
     .then(function (relations) {
       ...
       entities.forEach(function(entity){
         console.log(entity)
         // for each index
       return f.executeEsSearch(entity.label, "doc", esFuzzyBody, 10)
       .then(function (searchResults){
         console.log(searchResults)
       
         /**************************
          * Create id from each node in result to send on to Gramlin query and get nodes to be placed on graph
          * *************************/
       ...
       //   // console.log(entities)
       //   entities = _.sortBy(entities, 'label');
       //   var html = '<div>';
